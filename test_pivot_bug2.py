#!/usr/bin/env python3
"""Test to verify the pivot bug in rotation - with better visualization."""
...
def test_pivot_rotation():
    ...
    # Original point (x, y) in image coordinates becomes (height - y, x)
    # For pivot at (pivot_x * width, pivot_y * height):
    # After rotation: pivot is at (pivot_y * height, pivot_x * width) relative to new image
    # But we need to account for the expand=True which adds padding
    # 
    # Actually, the correct transformation for pivot fractions:
    # new_pivot_x = pivot_y (because height becomes width)
    # new_pivot_y = 1 - pivot_x (because width becomes height, and rotation flips)
    # 
    # Wait, let me think again...
    # 
    # For 90° counter-clockwise rotation with expand=True:
    # - Original (0,0) goes to (0, width) in new coordinates
    # - Original (width, 0) goes to (0, 0)
    # - Original (0, height) goes to (height, width)
    # - Original (width, height) goes to (height, 0)
    # 
    # So for a point (px, py) in original:
    # new_px = height - py
    # new_py = px
    # 
    # For pivot fractions:
    # new_pivot_x = (height - pivot_y * height) / height = 1 - pivot_y
    # new_pivot_y = pivot_x * width / width = pivot_x
    ...
    # Draw target pivot marker (yellow) on canvases FIRST
    draw_wrong = ImageDraw.Draw(canvas_wrong)
    draw_correct = ImageDraw.Draw(canvas_correct)
    draw_wrong.ellipse([target_x-5, target_y-5, target_x+5, target_y+5], fill=(255, 255, 0, 255))
    draw_correct.ellipse([target_x-5, target_y-5, target_x+5, target_y+5], fill=(255, 255, 0, 255))
